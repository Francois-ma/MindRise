...
export function SignupPanel() {
  ...
  const canSubmit = useMemo(() => form.name.trim().length > 1 && form.email.includes('@') && form.password.length >= 10 && form.accepts, [form]);
  ...
  async function submitVerification(event) {
    ...
    try {
      ...
      setVerified(true);
      setStatus({ type: 'success', message: 'Email verified. Your account is ready for the MindRise app.' });
    } catch (error) {
      setStatus({ type: 'error', message: error.message });
    } finally {
      setLoading(false);
        ...
        <form className="form-card" onSubmit={submitVerification}>
          <div className="verification-heading">
            ...
            <div>
              ...
              <p>{verified ? 'Your account can now sign in from the MindRise app.' : `Enter the six digit code sent to ${pendingEmail}.`}</p>
            </div>
          </div>
          {!verified && (
            <>
