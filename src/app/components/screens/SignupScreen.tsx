...
import { Mail, Lock, Eye, EyeOff, User, Heart, Sparkles, Check } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col px-4 pt-12 pb-8 max-w-md mx-auto bg-gradient-to-br from-green-50 via-yellow-50 to-white">
      ...
      <div className="mb-8 text-center">
        <div className="w-28 h-28 mx-auto mb-6 rounded-3xl overflow-hidden bg-white shadow-xl ring-4 ring-green-100">
          <img src={appIcon} alt="MindRise" className="w-full h-full object-cover" />
        </div>
        ...
        <p className="text-green-700/70">
          Create an account to start improving your mental wellness
        </p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {passwordStrength && (
                <div className="mt-2 flex gap-1">
                  <div className={`h-1 flex-1 rounded ${passwordStrength === 'weak' ? 'bg-red-500' : passwordStrength === 'medium' ? 'bg-orange-500' : 'bg-green-500'}`} />
                  <div className={`h-1 flex-1 rounded ${passwordStrength === 'medium' || passwordStrength === 'strong' ? 'bg-orange-500' : 'bg-gray-200'}`} />
                  <div className={`h-1 flex-1 rounded ${passwordStrength === 'strong' ? 'bg-green-500' : 'bg-gray-200'}`} />
                </div>
              )}
              {passwordStrength && (
                <p className={`text-xs mt-1 ${passwordStrength === 'weak' ? 'text-red-600' : passwordStrength === 'medium' ? 'text-orange-600' : 'text-green-600'}`}>
            ...
            <div className="flex items-start gap-2 pt-2">
              <Checkbox
                ...
                className="mt-0.5"
              />
              <label htmlFor="terms" className="text-sm cursor-pointer text-green-700/80">
                I agree to the{' '}
                <button type="button" className="text-green-700 hover:text-green-800 font-medium">
                  Terms & Conditions
                </button>{' '}
                and{' '}
                <button type="button" className="text-green-700 hover:text-green-800 font-medium">
